 ...
 jQuery(document).ready(function($) {
	...
	window.customdesign = {
		...
		itemInStage: function(action){
			customdesign.actions.do('price_action', action);
		},
				...
				drawControls: function(ctx) {
					...
					if (this.hasRotatingPoint) {
						if (active !== null && active !== undefined && active.get('lockPosition') === true) {
							...
							if(customdesign.ops.window_width >= 768 && (isgroup || isobj)){
								ctx.font = "12px Roboto";	
								var size = customdesign.get.obj_size(isgroup ?? isobj);
							...
							if (
								...
								var opts = customdesign.get.ptcfg_type(isobj);
		...
		objects : {
			...
			events : {
				...
				'object:selected' : function(opts) {
					...
					if (opts.target.get('lockPosition') === true) {
						...
						var ptcfg = customdesign.get.ptcfg_type(opts.target);
				...
				'object:scaling' : function(opts) {
					...
					if (opts.target && !opts.target.template) {
						...
						if(ptcfg && ptcfg.options != undefined){
							var min_scale = ptcfg.options.min_scale != undefined  && ptcfg.options.min_scale ? parseFloat(ptcfg.options.min_scale) : 0,
								...
								min_scaleX = 0,
								max_scaleX = 0,
								size = customdesign.get.obj_size(opts.target,'px');
					...
					customdesign.actions.do('object:scaling', opts);
				},
			...
			clipto : function (ctx, obj) {
				...
				obj.scaleX = customdesign.apply_filters('scale_designs_value_X', obj.scaleX);
				obj.scaleY = customdesign.apply_filters('scale_designs_value_Y', obj.scaleY);
			...
			text : function(ops){
				...
				if (
					...
					let ptcfg = customdesign.get.ptcfg_type('font');
					
					if(ptcfg && ptcfg.options.editable != undefined && (customdesign.data.rtl!= '1')){
					...
					if(ptcfg && ptcfg.options.min_text_letter != undefined && parseInt(ptcfg.options.min_text_letter) > text.length){
						customdesign.fn.notice(customdesign.i(215)+' '+ptcfg.options.min_text_letter, 'notice', 3500);
						return null;
					}

					if(ptcfg && ptcfg.options.max_text_letter != undefined && parseInt(ptcfg.options.max_text_letter) < text.length){
						customdesign.fn.notice(customdesign.i(214)+' '+ptcfg.options.max_text_letter, 'notice', 3500);
			...
			customdesign : {
				...
				'image' : function(ops, callback) {
					...
					img.onload = function() {
						...
						if (customdesign.fn.check_image_dimensions(this,ops) === false) 
						...
						ops.width = customdesign.apply_filters('object_size_width', ops.width, {iw: parseFloat(iw), ratio: ratio});
						ops.height = customdesign.apply_filters('object_size_height', ops.height, {ih: parseFloat(ih), ratio: ratio});
						...
						ops = customdesign.apply_filters('customdesign_image_object', ops);
		...
		design : {
			...
			events : function(){
				...
				customdesign.trigger({
					...
					leftNav: function(e) {
						...
						if (this.getAttribute('data-load')) {
							if (typeof customdesign.design.nav.load[this.getAttribute('data-load')] == 'function'){
								var ptcfg = customdesign.get.ptcfg_type(tab),
									data = {};
								...
								customdesign.design.nav.load[this.getAttribute('data-load')](e,data);
						...
						if (
							...
							customdesign.design.nav.callback[this.getAttribute('data-callback')](this,e);
					...
					templates_more: function(e) {
						...
						if (this.scrollTop + this.offsetHeight >= this.scrollHeight/* - 100*/) {
							var ptcfg = customdesign.get.ptcfg_type('templates'),
								data = {
									action: 'templates',
					...
					cliparts_more: function(e) {
						...
						if (this.scrollTop + this.offsetHeight >= this.scrollHeight/* - 100*/) {
							var ptcfg = customdesign.get.ptcfg_type('cliparts'),
								data = {
									action: 'cliparts',
								...
								switch (a.type) {
									...
									case 'i-text' :
										if(ptcfg && ptcfg.options.max_text_line && a._textLines && a._textLines.length > parseInt(ptcfg.options.max_text_line)){
											customdesign.fn.notice(customdesign.i(215)+' '+ptcfg.options.max_text_line, 'notice', 3500);
											e.preventDefault();
										}
										a.setText(this.value);
									break;
						...
						switch (func) {
							...
							case 'create-link' :
								...
								var wrp = $(e.target).closest('#customdesign-shares-wrp'),
									...
									screenshot = customdesign.data.stages[Object.keys(data.stages)[0]].screenshot;
				...
				.off('keydown').on('keydown', function(e) {
					...
					switch (e.keyCode) {
						case 8: return customdesign.actions.do('key-delete', e);
						case 46: return customdesign.actions.do('key-delete', e);
						case 13: return customdesign.actions.do('key-enter', e);
						case 27: return customdesign.actions.do('key-esc', e);
						case 37:
						case 38:
						case 39:
						case 40: return customdesign.actions.do('key-move', e);
					};

					if (e.metaKey === true || e.ctrlKey === true) {
						
						switch (e.keyCode) {
							case 48: return customdesign.actions.do('ctrl-0', e);
							case 65: return customdesign.actions.do('ctrl-a', e);
							case 68: return customdesign.actions.do('ctrl-d', e);
							case 69: return customdesign.actions.do('ctrl-e', e);
							case 79: return customdesign.actions.do('ctrl-o', e); case 80: return customdesign.actions.do('ctrl-p', e); case 83:
								if (e.shiftKey === true)
									return customdesign.actions.do('ctrl-shift-s', e);
								else return customdesign.actions.do('ctrl-s', e);
							case 90:
							...
							case 61: return customdesign.actions.do('ctrl+', e); case 173: return customdesign.actions.do('ctrl-', e); case 107: return customdesign.actions.do('ctrl+', e); case 109: return customdesign.actions.do('ctrl-', e); case 187: return customdesign.actions.do('ctrl+', e); case 189: return customdesign.actions.do('ctrl-', e);
						}
				...
				callback : {
							...
							wrp.find('>span').on('click', function(){
								...
								var next_step = function() {
									...
									var dataURLNew = new_group.toDataURL({
										multiplier: (psize.h/customdesign.stage().limit_zone.height)/(window.devicePixelRatio > 2 ? window.devicePixelRatio*2 : window.devicePixelRatio)
									});
					...
					printing: function(el, e){
						...
						var target =  el.getAttribute('data-tab'),
							colors = customdesign.data.colors,
							opts = customdesign.get.ptcfg_type(target);
		...
		xitems : {
			...
			load : function(comp, ops) {
				...
				var ptcfg = customdesign.get.ptcfg_type((comp=='images') ? 'image' : comp),
					data = {
					action: 'addon',
					component: comp,
					category: '',
				...
				customdesign.post(data, function(res) {
			...
			response: function (res, comp) {
				...
				if (res.items && res.items.length > 0) {
					res.items.map(function(item) {
						...
						var preview_type = '';
						if(customdesign.xitems.resources[comp].ops.preview != undefined && customdesign.xitems.resources[comp].ops.preview != '' && customdesign.xitems.resources[comp].ops.preview != 'undefined'){
							preview_type = customdesign.xitems.resources[comp].ops.preview;
						}
						...
						html += '<li style="background-image: url(\''+(
								...
								'}]\' class="customdesign-xitem">'+
									...
									(customdesign.ops.thumbn_preview ? '<i data-info="'+item.id+'"></i>' : '')+
								'</li>';
					});
				...
				var events = [['click', function(e){
					...
					if (customdesign.templates.storage[ops[0].id]) {
						customdesign.templates.load({
							...
							id: ops[0].id,
							price: customdesign.apply_filters('template_price', ops[0].price, ops)
						});
					...
					if(ptcfg && ptcfg.options != undefined){
						...
						if(min_width !== 0 || max_width !== 0){
							...
							if(tmp_size < min_width || tmp_size > max_width){
								...
								customdesign.fn.notice(customdesign.i(218)+' '+ max_width +'px', 'notice', 3500);
								return;
							}else{
						...
						if(min_scale !== 0)
							customdesign.fn.scale_designs(min_scale);
			...
			import : function(id, ops, dir) {
		    	var do_import = function() {
			    	...
			    	var html = '<li style="background-image: url(\''+URL.createObjectURL(ops.thumbn ? ops.thumbn : ops.url)+'\')" \
								...
								'+(customdesign.ops.thumbn_preview ? '<i data-info="'+id+'"></i>' : '')+'\
		...
		tools : {
			...
			set : function(obj) {
				...
				var opts = customdesign.get.ptcfg_type(obj);
				...
				if(opts && opts !== 'null' && opts != undefined){
						...
						Object.keys(opts.options).map(function(opt){
							...
							switch(opt){
								...
								case 'max_text_letter':
									el('workspace').find('.customdesign-edit-text').attr('maxlength',parseInt(opts.options[opt]));
									break;
								case 'min_text_letter':
									el('workspace').find('.customdesign-edit-text').attr('minlength',parseInt(opts.options[opt]));
									break;
								case 'filter':
					...
					if(opts.colors != undefined || opts.color_picker != undefined){
						var colors = (typeof opts.colors === 'undefined') ? customdesign.data.colors : opts.colors,
							enable_colors = (typeof opts.color_picker === 'undefined') ? customdesign.data.enable_colors : opts.color_picker;
						
						customdesign.render.colorPresets(colors,enable_colors);
			...
			import : function (data, callback) {
				...
				var stage = customdesign.stage(),
					...
					do_import = function(i) {
						...
						data.objects[i] = customdesign.apply_filters('before_import_object', data.objects[i]);
						...
						if (data.objects[i] !== undefined) {
							...
							if (customdesign.fn.url_var('cart', '') !== '') {
								...
								customdesign.data.stages[customdesign.render.stage_nav()].screenshot = customdesign.tools.toImage({
									stage: customdesign.data.stages[customdesign.render.stage_nav()],
								...
								// data.stages[s].screenshot = $('#customdesign-stage-nav ul li[data-stage="'+s+'"] img[data-stage="'+s+'"]').attr('src');
							}
					...
					Object.keys(data.stages).map(function(s){
						...
						if (s == customdesign.current_stage) {
						    ...
						    if (customdesign.data.stages[s] !== undefined) {
								...
								if (customdesign.data.stages[s].color !== undefined) {
									_stages[s].color = customdesign.data.stages[s].color;
								}
						    }
						}
		...
		get : {
			...
			design_size : function(stage, obj, unit = 'inch'){
				...
				if (typeof stage.size == 'string' &&  stage.size !== ''){
					Object.keys(customdesign.data.size_default).map(function(s) {
						if (customdesign.data.size_default[s].cm == stage.size) {
							px_size = customdesign.data.size_default[s].px.split('x');
							size = s.toLowerCase();
						}
					});
					...
					if (stage.size === undefined || stage.size === '') {
						let p_size = customdesign.get.el('print-nav').find('input[name="size"]').val();
						Object.keys(customdesign.data.size_default).map(function(s) {
							if (customdesign.data.size_default[s].cm == p_size) {
								size = s.toLowerCase();
							}
						});
						if(size === ''){
						...
						Object.keys(customdesign.data.size_default).map(function(s) {
							if (customdesign.data.size_default[s][stage.size.unit] == p_size)
								size = s.toLowerCase();
						});
						if(size === ''){
							if (stage.size.unit == 'inch'){
			...
			ptcfg_type : function(type) {
				if (customdesign.data.printings.length == 0) return;
				...
				var ptcfg = localStorage.getItem('CUSTOMDESIGN_PRINTING_CFG');
		...
		fn : {
			...
			product_color : function(color) {
				...
				sessionStorage.setItem("CUSTOMDESIGN-CHANGE-COLOR", "true");
				var stage = customdesign.stage();
				...
				if (stage.limit_zone) {
					...
					customdesign.tools.save();
					sessionStorage.removeItem("CUSTOMDESIGN-CHANGE-COLOR");
				}
			...
			set_svg_colors : function(obj) {
				...
				if(customdesign.get.el('svg-colors') && customdesign.get.el('svg-colors').data('editor'))
			...
			download_design : function (ops) {
				...
				switch (type) {
					...
					case 'json':
						...
						if(data != undefined && data.stages != undefined && data.stages.customdesign != undefined && data.stages.customdesign.data != undefined && data.stages.customdesign.data.objects != undefined && data.stages.customdesign.data.objects.length > 0){
							//console.log(JSON.stringify(data.stages.customdesign.data.objects));
							// $.each(data.stages.customdesign.data.objects, function(index, detailObj){
							data.stages.customdesign.data.objects.map(function(detailObj, index){
								if(detailObj.type == 'i-text' || detailObj.type == 'text-fx' || detailObj.type == 'curvedText'){
									...
									detailObj.text = res;
									data.stages.customdesign.data.objects[index] = detailObj;
								}
							});
						}
						customdesign.fn.download(
					...
					reader[f].addEventListener("load", function () {
						...
						if (url_data.indexOf('data:image/svg+xml;base64,') === 0) {
					        ...
					        img_preview_check.onload = function() {
					            let check_img = customdesign.fn.check_upload_dimensions(img_preview_check);
			...
			ctrl_btns : function(opts) {
				...
				if (corner == 'tl') {
					customdesign.itemInStage('remove');
					customdesign.tools.discard();
					customdesign.stack.save();
			...
			check_image_dimensions : function(img,ops) {
				...
				opts = customdesign.get.ptcfg_type(ops);
				...
				if(opts && opts.options != undefined){
					if (
						...
						if (
							...
							customdesign.fn.notice(customdesign.i(160)+' '+opts.options.min_dimensions.join('x'), 'notice', 3500);
							return false;
						}	
					};
					...
					if (
						...
						if (
							...
							customdesign.fn.notice(customdesign.i(216)+' '+opts.options.max_dimensions.join('x'), 'notice', 3500);
							return false;
						}
					}
					...
					if (typeof opts.options.min_dpi != undefined && opts.options.min_dpi !== '') {
						...
						if (
							...
							customdesign.fn.notice(customdesign.i(194)+' '+opts.options.min_dpi, 'notice', 3500);
							return false;
						}	
					};
					...
					if (typeof opts.options.max_dpi != undefined && opts.options.max_dpi !== '') {
						...
						if (
							...
							customdesign.fn.notice(customdesign.i(195)+' '+opts.options.max_dpi, 'notice', 3500);
							return false;
						}	
					};
					...
					// 		customdesign.fn.notice(customdesign.i(217)+' '+opts.options.min_width+'px', 'notice', 3500);
					// 		return false;
					// 	}
					// }
					...
					// 		customdesign.fn.notice(customdesign.i(218)+' '+opts.options.max_width+'px', 'notice', 3500);
					// 		return false;
					// 	}
					// }
			...
			update_state : function() {
				...
				customdesign.ops.preventDbl = setTimeout(function() {
					...
					Object.keys(customdesign.data.stages).map(function(s){
											...
											case 'curvedText':
												...
												if(_textLines != undefined  && _textLines.length > 0){
													...
													if(_textLines.length - 1 > 0){
														character[o.id] = _textLines.map((ele) => customdesign.fn.letterCount(ele))
															.reduce((a,b) => {
																for (let k in b) {
														...
														character[o.id] = customdesign.fn.letterCount(_textLines[0]);
													}
												}
												break;
											default:
						...
						if (customdesign.data.stages[s].canvas !== undefined){
							let stage = customdesign.data.stages[s],
								canvas = stage.canvas,
								active = customdesign.get.active(),
							...
							if (objects.length == 1) {
								sizes = $.extend(true, {}, customdesign.get.design_size(stage, objects[0]));
								//canvas.setActiveObject(objects[0]);
							}else if (objects.length > 1) {
								var group = new fabric.Group(objects,{});
								canvas._activeObject = null;
								canvas.setActiveGroup(group.setCoords()).renderAll();
								sizes = $.extend(true, {}, customdesign.get.design_size(stage, group));
					...
					//$('#customdesign-count-colors i').html(mo).css({background: 'linear-gradient(to right, '+bg.join(', ')+')'}).attr({title: 'Used '+mo+' colors'});
			...
			process_variations : function(values, el) {
				...
				if(customdesign.cart.printing.current != null){
					sessionStorage.setItem('CUSTOMDESIGN-PRINT-VALUE', customdesign.cart.printing.current);
				}
				customdesign.cart.printing.current = null;
			...
			keep_current_designs : function(new_stages) {
				...
				curent_designs = customdesign.apply_filters('filter_current_design', curent_designs, new_stages);
					...
					for (var i in print.calculate.values){
						...
						for (var r in 
							...
							if(r == 'ppu' && ['line','multi','character','size','color','acreage'].includes(print.calculate.type)){
								r = customdesign.data.print_types[print.calculate.type]['ppu'] || "Price";
							}
						...
						for (var r in print.calculate.values[i]){
							...
							for (var td in print.calculate.values[i][r]) {
								...
								table_content += '<td>' +
									((print.calculate.values[i][r][td]*1>0) ? 
									customdesign.fn.price(print.calculate.values[i][r][td]) :
									customdesign.fn.price(print.calculate.values[i][r][td]))+ '</td>';
							};	
							table_content += '</tr>';
						};
			...
			export : function(save, id, created, name) {
				...
				customdesign.get.el('stage-nav').find('li[data-stage]').each(function(i){
					...
					var activeStageStatus = $('#customdesign-stage-nav ul li[data-stage="'+s+'"]').hasClass('active');
					...
					if (stage.canvas) {
						...
						if(activeStageStatus == true && save != 'cart'){
							data.stages[s].screenshot = customdesign.tools.toImage({
								stage: stage,
								//is_bg: (save == 'cart' || save == 'share') ? 'full' : false, 
								is_bg: 'full', 
								multiplier: 1/window.devicePixelRatio
						...
						if (sessionStorage.getItem("CUSTOMDESIGN-CHANGE-COLOR") != null && save != 'cart') {
						  	data.stages[s].screenshot = customdesign.tools.toImage({
								stage: stage,
								//is_bg: (save == 'cart' || save == 'share') ? 'full' : false, 
								is_bg: 'full', 
							...
							if( (data.stages[s].screenshot == undefined || data.stages[s].screenshot == 'undefined' || data.stages[s].screenshot == null) && data.stages[s].image != ''){
								customdesign.data.stages[s].screenshot = data.stages[s].image;
							}
						}
						...
						if(save == 'cart'){
							...
							customdesign.data.stages[s].canvas.getObjects().map(function(obj){
								if(obj.evented === true){
			...
			cart_thumbn : function(id) {
                ...
                customdesign.indexed.get(id, 'cart', function(res){
                    ...
                    for(var i=0; i< objs.length; i++)
                    {
						...
						if( $('#customdesign-cart-items img[data-id="'+id+'"]').length > 0 && i == 0){
							$('#customdesign-cart-items img[data-id="'+id+'"]').attr('src', stage.screenshot);
						}
                    }
                });
            },
		...
		render : {
			...
			colorPresets : function(list_colors, enable_colors) {
				...
				var colors = (typeof list_colors === 'undefined') ? customdesign.data.colors : list_colors,
					enable_colors = (typeof enable_colors === 'undefined') ? customdesign.data.enable_colors : enable_colors,
					el = $('.customdesign-color-presets'),
					el_stroke = $('#customdesign-stroke-fix-colors'),
					lb;
				
				if (colors !== undefined && colors.indexOf(':') > -1)
					colors = colors.split(':')[1].replace(/\|/g, ',');
				
				if (enable_colors != '0' && localStorage.getItem('customdesign_color_presets' && !customdesign.cart.printing.current)) {
					colors = localStorage.getItem('customdesign_color_presets').replace(/\|/g, ',');
				};

				el.html('');
				
				if(customdesign.data.printings.length > 0 && customdesign.cart.printing.current != null) 
					el_stroke.html('');
				...
				colors.split(',').map(function(c){
					...
					if(customdesign.data.printings.length && customdesign.cart.printing.current != null)
				...
				if(
					customdesign.data.printings.length &&
					customdesign.cart.printing.current != null
				){
				...
				if (typeof stages == 'object') {
					...
					Object.keys(stages).map(function(s){
						...
						if (s != 'colors') {
							...
							if(customdesign.fn.url_var('product_base', '').indexOf('variable:') == -1){
								stages[s].color = customdesign.data._color;
							}

							var stageColor = customdesign.get.color();

							if(customdesign.fn.url_var('product_base', '').indexOf('variable:') != -1 && stages[s].color != undefined && stageColor != stages[s].color){
			...
			product : function(data, callback, designs) {
				...
				if(stagesBase64 == true){
					customdesign.data.color = Object.values(stageBase64Decode)[0].color;
				}
				...
				customdesign.active_stage(customdesign.render.stage_nav(), function(){
						...
						$.get(customdesign.data.upload_url+'shares/'+customdesign.data.share+'.lumi', function(res) {
							...
							if (res.stages !== undefined) {
								...
								customdesign.tools.imports(res, function(){
													...
													customdesign.fn.download_design({
														...
														callback: function(data) {
															...
															if (Object.keys(customdesign.data.stages)[start_render] !== undefined) {
																...
																customdesign.active_stage(first_stage);
																customdesign.data.stages[first_stage].screenshot = customdesign.tools.toImage({
																	stage: customdesign.data.stages[first_stage],
																	is_bg: 'full', 
																	multiplier: 1/window.devicePixelRatio
																});
																
																$('#customdesign-stage-nav img[data-stage="'+first_stage+'"]').attr({
																	src: customdesign.data.stages[first_stage].screenshot
																});
									...
									if (Object.keys(customdesign.data.stages)[start_render] !== undefined && Object.keys(customdesign.data.stages).length-1 > 0) {
					...
					// hash : 5fecb5125fdb9c9cf8f2e54802cfb020
					else if (customdesign.fn.url_var('cart', '') !== '' && sessionStorage.getItem("CUSTOMDESIGN-START-NEW") !== null && sessionStorage.getItem('CUSTOMDESIGN-START-NEW') === true) 
					{
					...
					} else if(customdesign.fn.url_var('cart', '') !== ''){
									...
									customdesign.active_stage(s, function() {
										...
										customdesign.fn.download_design({
											...
											callback: function(data) {
												...
												customdesign.actions.do('load_object_stages');
												
												if (Object.keys(customdesign.data.stages)[start_render] !== undefined) {
													// if(customdesign.data.stages[Object.keys(customdesign.data.stages)[start_render]].data == undefined){
													// 	start_render++;
													// }
													export_print_file (Object.keys(customdesign.data.stages)[start_render]);
												} else {
													customdesign.active_stage(Object.keys(customdesign.data.stages)[0]);
													customdesign.data.stages[first_stage].screenshot = customdesign.tools.toImage({
														stage: customdesign.data.stages[first_stage],
														is_bg: 'full', 
														multiplier: 1/window.devicePixelRatio
					...
					setTimeout(customdesign.fn.product_color, 1, customdesign.get.color());
			...
			layouts : function(data,callback){
				...
				if(customdesign.ops.first_completed && customdesign.cart.printing.current === null && typeof callback == 'function') 
					return callback();
					
				customdesign.f('Loading layout..');
				...
				if(data.printing !== undefined){
					$.ajax({
						...
						success: function(res) {
							if(res){
								customdesign.get.els = {};
								$.each( res, function( key, value ) {
									if ( key == 'left'){
										customdesign.get.el(key).html( $(value).html() );
										if($('#customdesign-layers', customdesign.get.el(key)).length > 0)
											customdesign.e.layers = $('#customdesign-layers', customdesign.get.el(key));
									}else if( key == 'navigations'){
										$('<div/>').html(value).find('ul[data-block]').each(function(index){
											$('ul[data-block]', customdesign.get.el(key)).eq(index).replaceWith(this);
										})
									}else{
										customdesign.get.el(key).html( value );
										if(key == 'top-tools'){
											jscolor.bind();
										}
									}													
								...
								customdesign.design.events();	
	
								customdesign.cart.init();
	
								customdesign.render.fonts();
					...
					Object.keys(customdesign.data.layouts).map(function(l) {
						if ( l == 'left'){
							customdesign.get.el(l).html( $(customdesign.data.layouts[l]).html());
						}else if( l == 'navigations'){
							$('<div/>').html(customdesign.data.layouts[l]).find('ul[data-block]').each(function(index){
								$('ul[data-block]', customdesign.get.el(l)).eq(index).replaceWith(this);
							})
						}else{
							customdesign.get.el(l).html( customdesign.data.layouts[l] );
							if(l == 'top-tools'){
								jscolor.bind();
							}
						}
					...
					if(sessionStorage.getItem('customdesign_change_product') !== null && customdesign.get.el('product').length > 0)
					...
					customdesign.ops.thumbn_preview = true;
				...
				if (res.items && res.items.length > 0) {
					res.items.map(function(item) {
						...
						html += '<li style="background-image: url(\''+item.screenshot+'\')" \
								...
								<i data-tag="'+item.id+'">'+(item.price?customdesign.fn.price(item.price) : customdesign.i(100))+'</i>'+(customdesign.ops.thumbn_preview ? '<i data-info="'+item.id+'"></i>' : '')+'\
								</li>';
					});
					var total = res.total ? res.total : 0;
				...
				if (res.items && res.items.length > 0) {
					...
					res.items.map(function(item) {
						...
						html += '<li style="background-image: url(\''+item.thumbnail_url+
									...
									'</i>'+(customdesign.ops.thumbn_preview ? '<i data-info="'+item.id+'"></i>' : '')+
								'</li>';
					});
			...
			list_products : function(res) {
				...
				if (res.products && res.products.length > 0) {
					...
					res.products.map(function(p) {
						...
						try {
							var attrs = customdesign.fn.dejson(p.attributes);
						} catch (ex) {
							var attrs = {};
						};	
						...
						if (product.length > 0) {
							if (typeof wrp.data('callback') == 'function') {
								...
								if(product[0].printings.length > 0)
									data.printing = sessionStorage["CUSTOMDESIGN-PRINT-VALUE"] ? sessionStorage["CUSTOMDESIGN-PRINT-VALUE"] : product[0].printings[0].id;
									...
									customdesign.fn.confirm({
										...
										primary: {
											...
											callback: function(e) {
												...
												customdesign.render.layouts(data, function(){
		...
		mobile : function(canvas_view) {
			...
			if (ww<450) {
				...
				var total_element = 0;
				$('ul.customdesign-left-nav li').each(function(){
		...
		active_stage : function(name, callback) {
			...
			if (stage.color != undefined && customdesign.fn.url_var('product_base', '').indexOf('variable:') != -1 && customdesign.data.color == undefined){
				customdesign.data.color = stage.color;
			}

			if(stage.template != undefined && stage.template.id != undefined && stage.data != undefined && stage.data.objects != undefined && customdesign.fn.url_var('design_print', '') == '' && customdesign.fn.url_var('order_print', '') == '' && customdesign.fn.url_var('cart', '') == '' ){
				delete stage.data;
			}
		...
		cart : {
            ...
            */
			...
			process_add_cart	: function(cart_design) {
				...
				cart_data[id] = {
					...
					attributes	: $.extend(true, {}, customdesign.ops.product_data.attributes),
					ext_attributes  : $.extend(true, {}, customdesign.ops.product_data.ext_attributes_value),
			...
			calc				: function (states_data) {
				...
				Object.keys(customdesign.data.stages).map(function(s){
					...
					if (typeof customdesign.data.stages[s].canvas !== 'undefined'){
						...
						canvas.getObjects().map(function (obj){
							// customdesign.actions.do('load_object_stages', obj);
							if (obj.evented == true) {
			...
			*/
			...
			render				: function (data) {
				...
				customdesign.actions.do('vari_data_price', data);
				...
				*/
				...
				product_base_check = customdesign.fn.url_var('product_base', '');
				// if(product_base_check.indexOf('variable:') != -1){
				// 	wrp.append(
				// 		'<div class="customdesign-cart-field field-inline ext-attribute">\
				...
				customdesign.trigger({
					...
					calc_cart : function (e){
						...
						if(product_base_check.indexOf('variable:') != -1){
							...
							// $.each(customdesign.ops.product_data.ext_list_variation, function(product_id, select_data){
							...
							//sessionStorage.setItem('CUSTOMDESIGN-ATTRIBUTE-VARIATION', JSON.stringify(attribute_save));
							...
							$('div.customdesign-cart-field.ext-attribute select').each(function(index, el ) {
								...
								// foreach list variation
								$.each(customdesign.ops.product_data.ext_list_variation, function(p_id, select_data){
									var match = true;
						...
						if(e.target.getAttribute('name') == 'quantity'){
							sessionStorage.setItem('CUSTOMDESIGN-CHANGE-QUANTITY', 'true');
						}
					...
					clear_variable : function(e){
						// foreach attribute
						$.each(customdesign.ops.product_data.ext_attributes, function(index, detail){
	        ...
	        printing			: {
				...
				render : function (active){
					...
					customdesign.data.printings.map(function (print, index){
							...
							select_printing : function (e){
								...
								if(match_print.length > 0){
									...
									if (typeof print.resource == 'string')
										print.resource = customdesign.fn.dejson(print.resource);
									...
									if (typeof print.resource == 'object'){
										Object.keys(print.resource).map(function(res){
											if(print.resource[res].hasOwnProperty('options') && typeof print.resource[res].options == 'string'){
												let options = customdesign.fn.dejson(print.resource[res].options);
									...
									if (typeof print.layout == 'object' && print.layout.actions){
										...
										customdesign.ops.thumbn_preview = print.layout.actions.indexOf('preview') === -1 ? false : true;
									}

									localStorage.setItem('CUSTOMDESIGN_PRINTING_CFG', JSON.stringify(ptcfg));
									customdesign.render.fonts();
								}	
								
								if(sessionStorage.getItem('customdesign_change_layout') !== null){
									sessionStorage.removeItem('customdesign_change_layout');
									return;
								}

								if (customdesign.ops.first_completed == true) {

									var values = { printing : customdesign.cart.printing.current};
									$('.customdesign-cart-attributes .customdesign-cart-param:not(.disabled)').serializeArray().map(function(x){
										values[x.name] = x.value;
									});
									...
									$.ajax({
										...
										success: function(res) {
											if(res){
												...
												sessionStorage.setItem('customdesign_change_layout','changed');
												var vari_data = customdesign.fn.process_variations(values, null);
												
												//customdesign.cart.render(vari_data);
												customdesign.render.product_attrs(vari_data);
						...
						if(sessionStorage["CUSTOMDESIGN-PRINT-VALUE"]){
							if(customdesign.data.printings.length > 0){
								$.each(customdesign.data.printings, function(index, detail){
									if(detail.id == sessionStorage["CUSTOMDESIGN-PRINT-VALUE"]){
										customdesign.cart.printing.current = parseInt(sessionStorage.getItem("CUSTOMDESIGN-PRINT-VALUE"));
										customdesign.cart.calc();
										sessionStorage.removeItem("CUSTOMDESIGN-PRINT-VALUE");
									}
								});
							}
						}
	            ...
	            calc : function (qty) {
					...
					for (var s in states_data){
						...
						if(
							...
							var //ptrcfg = customdesign.data.printings_cfg,
			...
			*/
            ...
            this.actions.add('key-delete', function(e) {
	            ...
	            customdesign.stack.save();
	         	...
	         	elms.map(function(el) {
					let	opts = customdesign.get.ptcfg_type(el);
					if(opts && opts.options.removable != undefined && !opts.options.removable)
						return false; 
		         	canvas.remove(el);
	         	});
			...
			this.actions.add('db-ready', function(){
				...
				if (has_cart === true) {
					...
					if(customdesign.data.onload.printings.length > 0){
						let data = {
								'printing' : sessionStorage["CUSTOMDESIGN-PRINT-VALUE"] ? sessionStorage["CUSTOMDESIGN-PRINT-VALUE"] : customdesign.data.onload.printings[0].id
							};
			...
			customdesign.add_filter('customdesign_image_object', (object) => {
				if(object !== undefined){
					...
					var stage = customdesign.stage(),
						...
						multiplier = psize.w/(stage.limit_zone.width-1),
						mp = psize.o != 'landscape' ? multiplier : multiplier*(canvas.width/canvas.height),
						ptcfg = customdesign.get.ptcfg_type(object),
						min_scale = 0.01,
						max_scaleX = 0,
