...
jQuery(document).ready(function($) {
	...
	window.magic = {
		...
		itemInStage: function(action){
			magic.actions.do('price_action', action);
		},
				...
				drawControls: function(ctx) {
					...
					if (this.hasRotatingPoint) {
						if (active !== null && active !== undefined && active.get('lockPosition') === true) {
							...
							if(magic.ops.window_width >= 768 && (isgroup || isobj)){
								ctx.font = "12px Roboto";	
								var size = magic.get.obj_size(isgroup ?? isobj);
							...
							if (
								...
								var opts = magic.get.ptcfg_type(isobj);
		...
		objects : {
			...
			events : {
				...
				'object:selected' : function(opts) {
					...
					if (opts.target.get('lockPosition') === true) {
						...
						var ptcfg = magic.get.ptcfg_type(opts.target);
				...
				'object:scaling' : function(opts) {
					...
					if (opts.target && !opts.target.template) {
						...
						if(ptcfg && ptcfg.options != undefined){
							var min_scale = ptcfg.options.min_scale != undefined  && ptcfg.options.min_scale ? parseFloat(ptcfg.options.min_scale) : 0,
								...
								min_scaleX = 0,
								max_scaleX = 0,
								size = magic.get.obj_size(opts.target,'px');
					...
					magic.actions.do('object:scaling', opts);
				},
			...
			clipto : function (ctx, obj) {
				...
				obj.scaleX = magic.apply_filters('scale_designs_value_X', obj.scaleX);
				obj.scaleY = magic.apply_filters('scale_designs_value_Y', obj.scaleY);
			...
			text : function(ops){
				...
				if (
					...
					let ptcfg = magic.get.ptcfg_type('font');
					
					if(ptcfg && ptcfg.options.editable != undefined && (magic.data.rtl!= '1')){
					...
					if(ptcfg && ptcfg.options.min_text_letter != undefined && parseInt(ptcfg.options.min_text_letter) > text.length){
						magic.fn.notice(magic.i(215)+' '+ptcfg.options.min_text_letter, 'notice', 3500);
						return null;
					}

					if(ptcfg && ptcfg.options.max_text_letter != undefined && parseInt(ptcfg.options.max_text_letter) < text.length){
						magic.fn.notice(magic.i(214)+' '+ptcfg.options.max_text_letter, 'notice', 3500);
			...
			magic : {
				...
				'image' : function(ops, callback) {
					...
					img.onload = function() {
						...
						if (magic.fn.check_image_dimensions(this,ops) === false) 
						...
						ops.width = magic.apply_filters('object_size_width', ops.width, {iw: parseFloat(iw), ratio: ratio});
						ops.height = magic.apply_filters('object_size_height', ops.height, {ih: parseFloat(ih), ratio: ratio});
						...
						ops = magic.apply_filters('magic_image_object', ops);
		...
		design : {
			...
			events : function(){
				...
				magic.trigger({
					...
					leftNav: function(e) {
						...
						if (this.getAttribute('data-load')) {
							if (typeof magic.design.nav.load[this.getAttribute('data-load')] == 'function'){
								var ptcfg = magic.get.ptcfg_type(tab),
									data = {};
								...
								magic.design.nav.load[this.getAttribute('data-load')](e,data);
						...
						if (
							...
							magic.design.nav.callback[this.getAttribute('data-callback')](this,e);
					...
					templates_more: function(e) {
						...
						if (this.scrollTop + this.offsetHeight >= this.scrollHeight/* - 100*/) {
							var ptcfg = magic.get.ptcfg_type('templates'),
								data = {
									action: 'templates',
					...
					cliparts_more: function(e) {
						...
						if (this.scrollTop + this.offsetHeight >= this.scrollHeight/* - 100*/) {
							var ptcfg = magic.get.ptcfg_type('cliparts'),
								data = {
									action: 'cliparts',
								...
								switch (a.type) {
									...
									case 'i-text' :
										if(ptcfg && ptcfg.options.max_text_line && a._textLines && a._textLines.length > parseInt(ptcfg.options.max_text_line)){
											magic.fn.notice(magic.i(215)+' '+ptcfg.options.max_text_line, 'notice', 3500);
											e.preventDefault();
										}
										a.setText(this.value);
									break;
						...
						switch (func) {
							...
							case 'create-link' :
								...
								var wrp = $(e.target).closest('#magic-shares-wrp'),
									...
									screenshot = magic.data.stages[Object.keys(data.stages)[0]].screenshot;
				...
				.off('keydown').on('keydown', function(e) {
					...
					switch (e.keyCode) {
						case 8: return magic.actions.do('key-delete', e);
						case 46: return magic.actions.do('key-delete', e);
						case 13: return magic.actions.do('key-enter', e);
						case 27: return magic.actions.do('key-esc', e);
						case 37:
						case 38:
						case 39:
						case 40: return magic.actions.do('key-move', e);
					};

					if (e.metaKey === true || e.ctrlKey === true) {
						
						switch (e.keyCode) {
							case 48: return magic.actions.do('ctrl-0', e);
							case 65: return magic.actions.do('ctrl-a', e);
							case 68: return magic.actions.do('ctrl-d', e);
							case 69: return magic.actions.do('ctrl-e', e);
							case 79: return magic.actions.do('ctrl-o', e); case 80: return magic.actions.do('ctrl-p', e); case 83:
								if (e.shiftKey === true)
									return magic.actions.do('ctrl-shift-s', e);
								else return magic.actions.do('ctrl-s', e);
							case 90:
							...
							case 61: return magic.actions.do('ctrl+', e); case 173: return magic.actions.do('ctrl-', e); case 107: return magic.actions.do('ctrl+', e); case 109: return magic.actions.do('ctrl-', e); case 187: return magic.actions.do('ctrl+', e); case 189: return magic.actions.do('ctrl-', e);
						}
				...
				callback : {
							...
							wrp.find('>span').on('click', function(){
								...
								var next_step = function() {
									...
									var dataURLNew = new_group.toDataURL({
										multiplier: (psize.h/magic.stage().limit_zone.height)/(window.devicePixelRatio > 2 ? window.devicePixelRatio*2 : window.devicePixelRatio)
									});
					...
					printing: function(el, e){
						...
						var target =  el.getAttribute('data-tab'),
							colors = magic.data.colors,
							opts = magic.get.ptcfg_type(target);
		...
		xitems : {
			...
			load : function(comp, ops) {
				...
				var ptcfg = magic.get.ptcfg_type((comp=='images') ? 'image' : comp),
					data = {
					action: 'addon',
					component: comp,
					category: '',
			...
			response: function (res, comp) {
				...
				if (res.items && res.items.length > 0) {
					res.items.map(function(item) {
						...
						var preview_type = '';
						if(magic.xitems.resources[comp].ops.preview != undefined && magic.xitems.resources[comp].ops.preview != '' && magic.xitems.resources[comp].ops.preview != 'undefined'){
							preview_type = magic.xitems.resources[comp].ops.preview;
						}
						...
						html += '<li style="background-image: url(\''+(
								...
								'}]\' class="magic-xitem">'+
									...
									(magic.ops.thumbn_preview ? '<i data-info="'+item.id+'"></i>' : '')+
								'</li>';
					});
				...
				var events = [['click', function(e){
					...
					if (magic.templates.storage[ops[0].id]) {
						magic.templates.load({
							...
							id: ops[0].id,
							price: magic.apply_filters('template_price', ops[0].price, ops)
						});
					...
					if(ptcfg && ptcfg.options != undefined){
						...
						if(min_width !== 0 || max_width !== 0){
							...
							if(tmp_size < min_width || tmp_size > max_width){
								...
								magic.fn.notice(magic.i(218)+' '+ max_width +'px', 'notice', 3500);
								return;
							}else{
						...
						if(min_scale !== 0)
							magic.fn.scale_designs(min_scale);
			...
			import : function(id, ops, dir) {
		    	var do_import = function() {
			    	...
			    	var html = '<li style="background-image: url(\''+URL.createObjectURL(ops.thumbn ? ops.thumbn : ops.url)+'\')" \
								...
								'+(magic.ops.thumbn_preview ? '<i data-info="'+id+'"></i>' : '')+'\
		...
		tools : {
			...
			set : function(obj) {
				...
				var opts = magic.get.ptcfg_type(obj);
				...
				if(opts && opts !== 'null' && opts != undefined){
						...
						Object.keys(opts.options).map(function(opt){
							...
							switch(opt){
								...
								case 'max_text_letter':
									el('workspace').find('.magic-edit-text').attr('maxlength',parseInt(opts.options[opt]));
									break;
								case 'min_text_letter':
									el('workspace').find('.magic-edit-text').attr('minlength',parseInt(opts.options[opt]));
									break;
								case 'filter':
					...
					if(opts.colors != undefined || opts.color_picker != undefined){
						var colors = (typeof opts.colors === 'undefined') ? magic.data.colors : opts.colors,
							enable_colors = (typeof opts.color_picker === 'undefined') ? magic.data.enable_colors : opts.color_picker;
						
						magic.render.colorPresets(colors,enable_colors);
			...
			import : function (data, callback) {
				...
				var stage = magic.stage(),
					...
					do_import = function(i) {
						...
						data.objects[i] = magic.apply_filters('before_import_object', data.objects[i]);
						...
						if (data.objects[i] !== undefined) {
							...
							if (magic.fn.url_var('cart', '') !== '') {
								...
								magic.data.stages[magic.render.stage_nav()].screenshot = magic.tools.toImage({
									stage: magic.data.stages[magic.render.stage_nav()],
								...
								// data.stages[s].screenshot = $('#magic-stage-nav ul li[data-stage="'+s+'"] img[data-stage="'+s+'"]').attr('src');
							}
					...
					Object.keys(data.stages).map(function(s){
						...
						if (s == magic.current_stage) {
						    ...
						    if (magic.data.stages[s] !== undefined) {
								...
								if (magic.data.stages[s].color !== undefined) {
									_stages[s].color = magic.data.stages[s].color;
								}
						    }
						}
		...
		get : {
			...
			design_size : function(stage, obj, unit = 'inch'){
				...
				if (typeof stage.size == 'string' &&  stage.size !== ''){
					Object.keys(magic.data.size_default).map(function(s) {
						if (magic.data.size_default[s].cm == stage.size) {
							px_size = magic.data.size_default[s].px.split('x');
							size = s.toLowerCase();
						}
					});
					...
					if (stage.size === undefined || stage.size === '') {
						let p_size = magic.get.el('print-nav').find('input[name="size"]').val();
						Object.keys(magic.data.size_default).map(function(s) {
							if (magic.data.size_default[s].cm == p_size) {
								size = s.toLowerCase();
							}
						});
						if(size === ''){
						...
						Object.keys(magic.data.size_default).map(function(s) {
							if (magic.data.size_default[s][stage.size.unit] == p_size)
								size = s.toLowerCase();
						});
						if(size === ''){
							if (stage.size.unit == 'inch'){
			...
			ptcfg_type : function(type) {
				if (magic.data.printings.length == 0) return;
				...
				var ptcfg = localStorage.getItem('MAGIC_PRINTING_CFG');
		...
		fn : {
			...
			product_color : function(color) {
				...
				sessionStorage.setItem("MAGIC-CHANGE-COLOR", "true");
				var stage = magic.stage();
				...
				if (stage.limit_zone) {
					...
					magic.tools.save();
					sessionStorage.removeItem("MAGIC-CHANGE-COLOR");
				}
			...
			set_svg_colors : function(obj) {
				...
				if(magic.get.el('svg-colors') && magic.get.el('svg-colors').data('editor'))
			...
			download_design : function (ops) {
				...
				switch (type) {
					...
					case 'json':
						...
						if(data != undefined && data.stages != undefined && data.stages.magic != undefined && data.stages.magic.data != undefined && data.stages.magic.data.objects != undefined && data.stages.magic.data.objects.length > 0){
							//console.log(JSON.stringify(data.stages.magic.data.objects));
							// $.each(data.stages.magic.data.objects, function(index, detailObj){
							data.stages.magic.data.objects.map(function(detailObj, index){
								if(detailObj.type == 'i-text' || detailObj.type == 'text-fx' || detailObj.type == 'curvedText'){
									...
									detailObj.text = res;
									data.stages.magic.data.objects[index] = detailObj;
								}
							});
						}
						magic.fn.download(
					...
					reader[f].addEventListener("load", function () {
						...
						if (url_data.indexOf('data:image/svg+xml;base64,') === 0) {
					        ...
					        img_preview_check.onload = function() {
					            let check_img = magic.fn.check_upload_dimensions(img_preview_check);
				...
				if(opts && opts.options != undefined){
					if (
						...
						if (
							...
							magic.fn.notice(magic.i(160)+' '+opts.options.min_dimensions.join('x'), 'notice', 3500);
							return false;
						}	
					};
					...
					if (
						...
						if (
							...
							magic.fn.notice(magic.i(216)+' '+opts.options.max_dimensions.join('x'), 'notice', 3500);
							return false;
						}
					}
					...
					if (typeof opts.options.min_dpi != undefined && opts.options.min_dpi !== '') {
						...
						if (
							...
							magic.fn.notice(magic.i(194)+' '+opts.options.min_dpi, 'notice', 3500);
							return false;
						}	
					};
					...
					if (typeof opts.options.max_dpi != undefined && opts.options.max_dpi !== '') {
						...
						if (
							...
							magic.fn.notice(magic.i(195)+' '+opts.options.max_dpi, 'notice', 3500);
							return false;
						}	
					};
					...
					// 		magic.fn.notice(magic.i(217)+' '+opts.options.min_width+'px', 'notice', 3500);
					// 		return false;
					// 	}
					// }
					...
					// 		magic.fn.notice(magic.i(218)+' '+opts.options.max_width+'px', 'notice', 3500);
					// 		return false;
					// 	}
					// }
			...
			update_state : function() {
				...
				magic.ops.preventDbl = setTimeout(function() {
					...
					Object.keys(magic.data.stages).map(function(s){
											...
											case 'curvedText':
												...
												if(_textLines != undefined  && _textLines.length > 0){
													...
													if(_textLines.length - 1 > 0){
														character[o.id] = _textLines.map((ele) => magic.fn.letterCount(ele))
															.reduce((a,b) => {
																for (let k in b) {
														...
														character[o.id] = magic.fn.letterCount(_textLines[0]);
													}
												}
												break;
											default:
						...
						if (magic.data.stages[s].canvas !== undefined){
							let stage = magic.data.stages[s],
								canvas = stage.canvas,
								active = magic.get.active(),
							...
							if (objects.length == 1) {
								sizes = $.extend(true, {}, magic.get.design_size(stage, objects[0]));
								//canvas.setActiveObject(objects[0]);
							}else if (objects.length > 1) {
								var group = new fabric.Group(objects,{});
								canvas._activeObject = null;
								canvas.setActiveGroup(group.setCoords()).renderAll();
								sizes = $.extend(true, {}, magic.get.design_size(stage, group));
					...
					//$('#magic-count-colors i').html(mo).css({background: 'linear-gradient(to right, '+bg.join(', ')+')'}).attr({title: 'Used '+mo+' colors'});
			...
			process_variations : function(values, el) {
				...
				if(magic.cart.printing.current != null){
					sessionStorage.setItem('MAGIC-PRINT-VALUE', magic.cart.printing.current);
				}
				magic.cart.printing.current = null;
			...
			keep_current_designs : function(new_stages) {
				...
				curent_designs = magic.apply_filters('filter_current_design', curent_designs, new_stages);
					...
					for (var i in print.calculate.values){
						...
						for (var r in 
							...
							if(r == 'ppu' && ['line','multi','character','size','color','acreage'].includes(print.calculate.type)){
								r = magic.data.print_types[print.calculate.type]['ppu'] || "Price";
							}
						...
						for (var r in print.calculate.values[i]){
							...
							for (var td in print.calculate.values[i][r]) {
								...
								table_content += '<td>' +
									((print.calculate.values[i][r][td]*1>0) ? 
									magic.fn.price(print.calculate.values[i][r][td]) :
									magic.fn.price(print.calculate.values[i][r][td]))+ '</td>';
							};	
							table_content += '</tr>';
						};
			...
			export : function(save, id, created, name) {
				...
				magic.get.el('stage-nav').find('li[data-stage]').each(function(i){
					...
					var activeStageStatus = $('#magic-stage-nav ul li[data-stage="'+s+'"]').hasClass('active');
					...
					if (stage.canvas) {
						...
						if(activeStageStatus == true && save != 'cart'){
							data.stages[s].screenshot = magic.tools.toImage({
								stage: stage,
								//is_bg: (save == 'cart' || save == 'share') ? 'full' : false, 
								is_bg: 'full', 
								multiplier: 1/window.devicePixelRatio
						...
						if (sessionStorage.getItem("MAGIC-CHANGE-COLOR") != null && save != 'cart') {
						  	data.stages[s].screenshot = magic.tools.toImage({
								stage: stage,
								//is_bg: (save == 'cart' || save == 'share') ? 'full' : false, 
								is_bg: 'full', 
							...
							if( (data.stages[s].screenshot == undefined || data.stages[s].screenshot == 'undefined' || data.stages[s].screenshot == null) && data.stages[s].image != ''){
								magic.data.stages[s].screenshot = data.stages[s].image;
							}
						}
						...
						if(save == 'cart'){
							...
							magic.data.stages[s].canvas.getObjects().map(function(obj){
								if(obj.evented === true){
			...
			cart_thumbn : function(id) {
                ...
                magic.indexed.get(id, 'cart', function(res){
                    ...
                    for(var i=0; i< objs.length; i++)
                    {
						...
						if( $('#magic-cart-items img[data-id="'+id+'"]').length > 0 && i == 0){
							$('#magic-cart-items img[data-id="'+id+'"]').attr('src', stage.screenshot);
						}
                    }
                });
            },
		...
		render : {
			...
			colorPresets : function(list_colors, enable_colors) {
				...
				var colors = (typeof list_colors === 'undefined') ? magic.data.colors : list_colors,
					enable_colors = (typeof enable_colors === 'undefined') ? magic.data.enable_colors : enable_colors,
					el = $('.magic-color-presets'),
					el_stroke = $('#magic-stroke-fix-colors'),
					lb;
				
				if (colors !== undefined && colors.indexOf(':') > -1)
					colors = colors.split(':')[1].replace(/\|/g, ',');
				
				if (enable_colors != '0' && localStorage.getItem('magic_color_presets' && !magic.cart.printing.current)) {
					colors = localStorage.getItem('magic_color_presets').replace(/\|/g, ',');
				};

				el.html('');
				
				if(magic.data.printings.length > 0 && magic.cart.printing.current != null) 
					el_stroke.html('');
				...
				colors.split(',').map(function(c){
					...
					if(magic.data.printings.length && magic.cart.printing.current != null)
				...
				if (typeof stages == 'object') {
					...
					Object.keys(stages).map(function(s){
						...
						if (s != 'colors') {
							...
							if(magic.fn.url_var('product_base', '').indexOf('variable:') == -1){
								stages[s].color = magic.data._color;
							}

							var stageColor = magic.get.color();

							if(magic.fn.url_var('product_base', '').indexOf('variable:') != -1 && stages[s].color != undefined && stageColor != stages[s].color){
			...
			product : function(data, callback, designs) {
				...
				if(stagesBase64 == true){
					magic.data.color = Object.values(stageBase64Decode)[0].color;
				}
				...
				magic.active_stage(magic.render.stage_nav(), function(){
						...
						$.get(magic.data.upload_url+'shares/'+magic.data.share+'.lumi', function(res) {
							...
							if (res.stages !== undefined) {
								...
								magic.tools.imports(res, function(){
													...
													magic.fn.download_design({
														...
														callback: function(data) {
															...
															if (Object.keys(magic.data.stages)[start_render] !== undefined) {
																...
																magic.active_stage(first_stage);
																magic.data.stages[first_stage].screenshot = magic.tools.toImage({
																	stage: magic.data.stages[first_stage],
																	is_bg: 'full', 
																	multiplier: 1/window.devicePixelRatio
																});
																
																$('#magic-stage-nav img[data-stage="'+first_stage+'"]').attr({
																	src: magic.data.stages[first_stage].screenshot
																});
									...
									if (Object.keys(magic.data.stages)[start_render] !== undefined && Object.keys(magic.data.stages).length-1 > 0) {
					...
					// hash : 5fecb5125fdb9c9cf8f2e54802cfb020
					else if (magic.fn.url_var('cart', '') !== '' && sessionStorage.getItem("MAGIC-START-NEW") !== null && sessionStorage.getItem('MAGIC-START-NEW') === true) 
					{
									...
									magic.active_stage(s, function() {
										...
										magic.fn.download_design({
											...
											callback: function(data) {
												...
												magic.actions.do('load_object_stages');
												
												if (Object.keys(magic.data.stages)[start_render] !== undefined) {
													// if(magic.data.stages[Object.keys(magic.data.stages)[start_render]].data == undefined){
													// 	start_render++;
													// }
													export_print_file (Object.keys(magic.data.stages)[start_render]);
												} else {
													magic.active_stage(Object.keys(magic.data.stages)[0]);
													magic.data.stages[first_stage].screenshot = magic.tools.toImage({
														stage: magic.data.stages[first_stage],
														is_bg: 'full', 
														multiplier: 1/window.devicePixelRatio
					...
					setTimeout(magic.fn.product_color, 1, magic.get.color());
			...
			layouts : function(data,callback){
				...
				if(magic.ops.first_completed && magic.cart.printing.current === null && typeof callback == 'function') 
					return callback();
					
				magic.f('Loading layout..');
				...
				if(data.printing !== undefined){
						...
						success: function(res) {
							if(res){
								...
								$.each( res, function( key, value ) {
									if ( key == 'left'){
										magic.get.el(key).html( $(value).html() );
										if($('#magic-layers', magic.get.el(key)).length > 0)
											magic.e.layers = $('#magic-layers', magic.get.el(key));
									}else if( key == 'navigations'){
										$('<div/>').html(value).find('ul[data-block]').each(function(index){
											$('ul[data-block]', magic.get.el(key)).eq(index).replaceWith(this);
										})
									}else{
										magic.get.el(key).html( value );
										if(key == 'top-tools'){
											jscolor.bind();
										}
									}													
					...
					Object.keys(magic.data.layouts).map(function(l) {
						if ( l == 'left'){
							magic.get.el(l).html( $(magic.data.layouts[l]).html());
						}else if( l == 'navigations'){
							$('<div/>').html(magic.data.layouts[l]).find('ul[data-block]').each(function(index){
								$('ul[data-block]', magic.get.el(l)).eq(index).replaceWith(this);
							})
						}else{
							magic.get.el(l).html( magic.data.layouts[l] );
							if(l == 'top-tools'){
								jscolor.bind();
							}
						}
					...
					if(sessionStorage.getItem('magic_change_product') !== null && magic.get.el('product').length > 0)
						$('#magic-left .magic-left-nav li[data-tab="product"]').trigger('click');
					else 
						$('#magic-left ul.magic-left-nav>li[data-tab]').eq(1).click();

					magic.ops.thumbn_preview = true;
				...
				if (res.items && res.items.length > 0) {
					res.items.map(function(item) {
						...
						html += '<li style="background-image: url(\''+item.screenshot+'\')" \
								...
								<i data-tag="'+item.id+'">'+(item.price?magic.fn.price(item.price) : magic.i(100))+'</i>'+(magic.ops.thumbn_preview ? '<i data-info="'+item.id+'"></i>' : '')+'\
								</li>';
					});
					var total = res.total ? res.total : 0;
				...
				if (res.items && res.items.length > 0) {
					...
					res.items.map(function(item) {
						...
						html += '<li style="background-image: url(\''+item.thumbnail_url+
									...
									'</i>'+(magic.ops.thumbn_preview ? '<i data-info="'+item.id+'"></i>' : '')+
								'</li>';
					});
			...
			list_products : function(res) {
				...
				if (res.products && res.products.length > 0) {
					...
					res.products.map(function(p) {
						...
						try {
							var attrs = magic.fn.dejson(p.attributes);
						} catch (ex) {
							var attrs = {};
						};	
						...
						if (product.length > 0) {
							if (typeof wrp.data('callback') == 'function') {
								...
								if(product[0].printings.length > 0)
									data.printing = sessionStorage["MAGIC-PRINT-VALUE"] ? sessionStorage["MAGIC-PRINT-VALUE"] : product[0].printings[0].id;
									...
									magic.fn.confirm({
										...
										primary: {
											...
											callback: function(e) {
												$('magic-general-status').html('');
												magic.fn.clear_url([]);
												magic.render.layouts(data, function(){
		...
		mobile : function(canvas_view) {
			...
			if (ww<450) {
				...
				var total_element = 0;
				$('ul.magic-left-nav li').each(function(){
		...
		active_stage : function(name, callback) {
			...
			if (stage.color != undefined && magic.fn.url_var('product_base', '').indexOf('variable:') != -1 && magic.data.color == undefined){
				magic.data.color = stage.color;
			}

			if(stage.template != undefined && stage.template.id != undefined && stage.data != undefined && stage.data.objects != undefined && magic.fn.url_var('design_print', '') == '' && magic.fn.url_var('order_print', '') == '' && magic.fn.url_var('cart', '') == '' ){
				delete stage.data;
			}
		...
		cart : {
            ...
            */
			...
			process_add_cart	: function(cart_design) {
				...
				cart_data[id] = {
					...
					attributes	: $.extend(true, {}, magic.ops.product_data.attributes),
					ext_attributes  : $.extend(true, {}, magic.ops.product_data.ext_attributes_value),
			...
			calc				: function (states_data) {
				...
				Object.keys(magic.data.stages).map(function(s){
					...
					if (typeof magic.data.stages[s].canvas !== 'undefined'){
						...
						canvas.getObjects().map(function (obj){
							// magic.actions.do('load_object_stages', obj);
							if (obj.evented == true) {
			...
			*/
			...
			render				: function (data) {
				...
				magic.actions.do('vari_data_price', data);
				...
				*/
				...
				product_base_check = magic.fn.url_var('product_base', '');
				// if(product_base_check.indexOf('variable:') != -1){
				// 	wrp.append(
				// 		'<div class="magic-cart-field field-inline ext-attribute">\
				...
				magic.trigger({
					...
					calc_cart : function (e){
						...
						if(product_base_check.indexOf('variable:') != -1){
							...
							// $.each(magic.ops.product_data.ext_list_variation, function(product_id, select_data){
							...
							//sessionStorage.setItem('MAGIC-ATTRIBUTE-VARIATION', JSON.stringify(attribute_save));
							...
							$('div.magic-cart-field.ext-attribute select').each(function(index, el ) {
						...
						if(e.target.getAttribute('name') == 'quantity'){
							sessionStorage.setItem('MAGIC-CHANGE-QUANTITY', 'true');
						}
					...
					clear_variable : function(e){
						// foreach attribute
						$.each(magic.ops.product_data.ext_attributes, function(index, detail){
	        ...
	        printing			: {
				...
				render : function (active){
					...
					magic.data.printings.map(function (print, index){
							...
							select_printing : function (e){
								...
								if(match_print.length > 0){
									...
									if (typeof print.resource == 'string')
										print.resource = magic.fn.dejson(print.resource);
									...
									if (typeof print.resource == 'object'){
										Object.keys(print.resource).map(function(res){
											if(print.resource[res].hasOwnProperty('options') && typeof print.resource[res].options == 'string'){
												let options = magic.fn.dejson(print.resource[res].options);
									...
									if (typeof print.layout == 'object' && print.layout.actions){
										...
										magic.ops.thumbn_preview = print.layout.actions.indexOf('preview') === -1 ? false : true;
									}

									localStorage.setItem('MAGIC_PRINTING_CFG', JSON.stringify(ptcfg));
									magic.render.fonts();
								}	
								
								if(sessionStorage.getItem('magic_change_layout') !== null){
									sessionStorage.removeItem('magic_change_layout');
									return;
								}

								if (magic.ops.first_completed == true) {

									var values = { printing : magic.cart.printing.current};
									$('.magic-cart-attributes .magic-cart-param:not(.disabled)').serializeArray().map(function(x){
										values[x.name] = x.value;
									});
									...
									$.ajax({
										...
										success: function(res) {
											if(res){
												...
												sessionStorage.setItem('magic_change_layout','changed');
												var vari_data = magic.fn.process_variations(values, null);
												
												//magic.cart.render(vari_data);
												magic.render.product_attrs(vari_data);
						...
						if(sessionStorage["MAGIC-PRINT-VALUE"]){
							if(magic.data.printings.length > 0){
								$.each(magic.data.printings, function(index, detail){
									if(detail.id == sessionStorage["MAGIC-PRINT-VALUE"]){
										magic.cart.printing.current = parseInt(sessionStorage.getItem("MAGIC-PRINT-VALUE"));
										magic.cart.calc();
										sessionStorage.removeItem("MAGIC-PRINT-VALUE");
									}
								});
							}
						}
	            ...
	            calc : function (qty) {
					...
					for (var s in states_data){
						...
						if(
							...
							var //ptrcfg = magic.data.printings_cfg,
			...
			*/
            ...
            this.actions.add('key-delete', function(e) {
	            ...
	            magic.stack.save();
	         	...
	         	elms.map(function(el) {
					let	opts = magic.get.ptcfg_type(el);
					if(opts && opts.options.removable != undefined && !opts.options.removable)
						return false; 
		         	canvas.remove(el);
	         	});
			...
			this.actions.add('db-ready', function(){
				...
				if (has_cart === true) {
					...
					if(magic.data.onload.printings.length > 0){
						let data = {
								'printing' : sessionStorage["MAGIC-PRINT-VALUE"] ? sessionStorage["MAGIC-PRINT-VALUE"] : magic.data.onload.printings[0].id
							};
			...
			magic.add_filter('magic_image_object', (object) => {
				if(object !== undefined){
					...
					var stage = magic.stage(),
						...
						multiplier = psize.w/(stage.limit_zone.width-1),
						mp = psize.o != 'landscape' ? multiplier : multiplier*(canvas.width/canvas.height),
						ptcfg = magic.get.ptcfg_type(object),
						min_scale = 0.01,
						max_scaleX = 0,
