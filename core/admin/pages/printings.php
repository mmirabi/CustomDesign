<?php
	...
	if (isset($_POST['action_submit']) && !empty($_POST['action_submit'])) {
		...
		$customdesign_admin->check_caps('printings');
		
		foreach ($val as $value) {

			$dt = $customdesign_admin->get_row_id($value, 'printings');
			switch ($data_action) {
				...
				case 'active':
					...
					$dt = $customdesign_admin->edit_row( $value, $data, 'printings' );
					break;
				case 'deactive':
					$data = array(
						'active' => 0
				...
				case 'delete':
					...
					if (!empty($dt['upload'])) {
						if (file_exists($tar_file.$dt['upload'])) {
							...
							unlink(str_replace(array($customdesign->cfg->upload_url, '/'), array($tar_file, TS), $dt['thumbnail']));
						}
					}

					$customdesign_admin->delete_row($value, 'printings');
					break;
				default:
					break;
	...
	$printings = $customdesign_admin->get_rows('printings', $search_filter, $orderby, $ordering, $per_page, $start);
	$total_record = $customdesign_admin->get_rows_total('printings');
    ...
    $config = array(
		...
		'total_page'    => $printings['total_page'],
 	    'limit'         => $per_page,
	    'link_full'     => $customdesign->cfg->admin_url.'customdesign-page=printings&tpage={page}',
	    'link_first'    => $customdesign->cfg->admin_url.'customdesign-page=printings',
	);
...
<div class="customdesign_wrapper">
	...
	<div class="customdesign_content">
		...
		<div class="customdesign_header">
			<h2><?php echo $customdesign->lang('Printings'); ?></h2>
			<a href="<?php echo $customdesign->cfg->admin_url;?>customdesign-page=printing" class="add-new customdesign-button">
				<i class="fa fa-plus"></i> 
				<?php echo $customdesign->lang('Add new printing'); ?>
			</a>
			<?php
		...
		<div class="customdesign_option">
			<div class="left">
				<form action="<?php echo $customdesign->cfg->admin_url;?>customdesign-page=printings" method="post">
			...
			<div class="right">
				<form action="<?php echo $customdesign->cfg->admin_url;?>customdesign-page=printings" method="post">
					...
					<input  class="customdesign_submit" type="submit" name="search_printing" value="<?php echo $customdesign->lang('Search'); ?>">
					<?php $customdesign->securityFrom();?>
		...
		<?php if ( isset($printings['total_count']) && $printings['total_count'] > 0) { ?>
			<div class="customdesign_wrap_table">
				<table class="customdesign_table customdesign_printings">
					<thead>
						<tr>
							...
							if ( is_array($printings['rows']) && count($printings['rows']) > 0 ) {
								...
								foreach ($printings['rows'] as $value) { ?>
									...
									<tr>
										...
										<td>
											<a href="<?php echo $customdesign->cfg->admin_url;?>customdesign-page=printing&id=<?php if(isset($value['id'])) echo $value['id'] ?>" class="name"><?php if(isset($value['title'])) echo $value['title']; ?></a>
										...
										<td>
											<a href="#" class="customdesign_action" data-type="printings" data-action="switch_active" data-status="<?php echo (isset($value['active']) ? $value['active'] : '0'); ?>" data-id="<?php if(isset($value['id'])) echo $value['id'] ?>">
												<?php
		...
		<?php } else {
					if (isset($total_record) && $total_record > 0) {
						...
						$_SESSION[$prefix.'data_search'] = '';
						echo '<a href="'.$customdesign->cfg->admin_url.'customdesign-page=printings" class="btn-back"><i class="fa fa-reply" aria-hidden="true"></i>'.$customdesign->lang('Back To Lists').'</a>';
					}
					else
						echo '<p class="no-data">'.$customdesign->lang('No data. Please add printing.').'</p>';
			}?>
