jQuery(document).ready(function($){
	...
	var wrp = $('#customdesign-popup'),
		...
		doing = function() {
			if (done < total) {
				$('#customdesign-cliparts-bundle-stt').html('<i class="fa fa-spinner fa-spin fa-fw"></i> Processing '+done+'/'+total);
			} else {
				$('#customdesign-cliparts-bundle-stt').html('Processed '+done+'/'+total);
			}
		},
		...
		process_files = function(files) {
			...
			for (var i=0; i<files.length; i++) {
				...
				if (customdesign_validate_file(files[i])) {
					...
					reader[total].addEventListener("load", function () {
						...
						$('#customdesign-list-categories input[type="checkbox"]:checked').map(function(){
						...
						var data = {
							upload: {
								...
								size: f.size,
								name: 'customdesign-base-'+f.name.replace(/[^0-9a-zA-Z\.\-\_]/g, "").trim().replace(/\ /g, '+'),
							},
							name: name.join('.'),
							cates: cates,
							tags: $('#customdesign-cliparts-tags').val().split(','),
							price: $('#customdesign-cliparts-price').val(),
							featured: $('#customdesign-cliparts-featured').get(0).checked
						};
							...
							callback: function(res) {
						    	...
						    	$.ajax({
									...
									data: CustomdesignDesign.filter_ajax({
										action: 'add_clipart',
										nonce: 'CUSTOMDESIGN_ADMIN_cliparts:'+nonce,
									...
									success: function(data){
										if(typeof data == "object")
										{
											...
											{
												$('#customdesign-cliparts-bundle-stt').html(data.error);
											}
											else 
											{
												$('#customdesign-cliparts-bundle-stt').html('Can\t upload at this time');
											}
										}
										else 
										{
		...
		reload_categories = function(data){
			...
			$('#customdesign-list-categories').html('<br><center><i class="fa fa-spinner fa-2x fa-spin fa-fw"></i></center><br>');
			...
			$.ajax({
				...
				success: function(res){
					...
					if (res.length > 0) {
						...
						checkbox.push('<p class="no-data">' + customdesign.i(89) +'</p>');
					}

					$('#customdesign-list-categories').html(checkbox.join(''));
					$('#customdesign-parent-categories').html(select.join(''));
	...
	$('#customdesign-add-bundle-cliparts').on('click', function(e){
		show();
		e.preventDefault();
		return false;
	});
	...
	$('#customdesign-upload-form').on('click', function() {
		$(this).find('input[type="file"]').get(0).click();
	});

	$('#customdesign-upload-form input[type="file"]').on('change', function(e){
	...
	$('#create-category-form button.customdesign-btn-primary').on('click', function(e){
		...
		if (
			$('#customdesign-list-categories label').filter(function(){
